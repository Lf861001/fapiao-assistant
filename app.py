...
def compress_invoice_numbers(invoice_numbers: list[str]) -> str:
    ...
    indexed_numbers = [(int(item), item) for item in invoice_numbers]
...
def clear_output():
    prepare_folders()
    return jsonify({"ok": True, "message": "输出文件夹已清空。"})
...
def generate_pdf():
    ...
    invoice_numbers = [clean_part(item) for item in request.form.getlist("invoiceNumbers[]")]
    invoice_numbers = [item for item in invoice_numbers if item]
    ...
    if not invoice_numbers:
        return jsonify({"ok": False, "message": "请至少填写一个发票号。"}), 400
    ...
    if len(invoice_numbers) != invoice_file_count:
        return jsonify(
            {
                "ok": False,
                "message": f"发票号数量必须和发票 PDF 数量一致。当前发票号 {len(invoice_numbers)} 个，发票 PDF {invoice_file_count} 个。",
            }
        ), 400
...
@app.route("/api/files")
def list_files():
    files = sorted(path.name for path in OUTPUT_DIR.glob("*.pdf"))
...
def download_all():
    ...
    zip_path = OUTPUT_DIR / "fapiao_output.zip"
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as archive:
        for pdf_file in pdf_files:
            archive.write(pdf_file, arcname=pdf_file.name)

    return send_file(zip_path, as_attachment=True, download_name="fapiao_output.zip")
